...
export default function Analytics() {
  ...
  const { data: shifts = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("shifts").select("*").eq("is_draft", false)
  ...
  const shiftTypeData = [
    { name: t("shift.morning"), value: shifts.filter((s) => s.type === "morning").length, color: SHIFT_COLORS.morning },
    { name: t("shift.evening"), value: shifts.filter((s) => s.type === "evening").length, color: SHIFT_COLORS.evening },
    { name: t("shift.night"), value: shifts.filter((s) => s.type === "night").length, color: SHIFT_COLORS.night },
  ];
  ...
  const weeklyData = weeks.map((weekStart) => {
    ...
    return {
      week: formatLocale(weekStart, "MMM d", locale),
          ...
          <SelectContent>
            {[0, 1, 2, 3, 4, 5].map((i) => (
              <SelectItem key={i} value={String(i)}>
                {formatLocale(subMonths(now, i), "MMMM yyyy", locale)}
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">{t("analytics.activeStaff")}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">{t("analytics.publishedShifts")}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">{t("analytics.avgFulfillment")}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">{t("analytics.missingRN")}</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-base">{t("analytics.weeklyDistribution")}</CardTitle>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={weeklyData}>
                ...
                <Tooltip />
                <Bar dataKey="morning" stackId="a" fill={SHIFT_COLORS.morning} name={t("shift.morning")} radius={[0, 0, 0, 0]} />
                <Bar dataKey="evening" stackId="a" fill={SHIFT_COLORS.evening} name={t("shift.evening")} />
                <Bar dataKey="night" stackId="a" fill={SHIFT_COLORS.night} name={t("shift.night")} radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-base">{t("analytics.shiftBreakdown")}</CardTitle>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={240}>
              <PieChart>
              ...
              <div key={s.id} className="space-y-2">
                <div className="flex items-center justify-between text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-muted-foreground">({(s.target_fte_percent * 100).toFixed(0)}% FTE)</span>
                  </div>
                ...
                <div className="flex items-center gap-2">
                  <Progress value={Math.min(s.fulfillment, 100)} className="h-2.5 flex-1" />
                  <Badge variant="outline" className={`text-xs w-14 justify-center ${
                    s.fulfillment >= 90 ? "bg-green-100 text-green-800 border-green-200"
                      : s.fulfillment >= 70 ? "bg-yellow-100 text-yellow-800 border-yellow-200"
